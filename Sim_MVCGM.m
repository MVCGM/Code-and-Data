function [fM1,fphi1,fZe1,fM2,fphi2,fZe2,LM] = Sim_MVCGM(fM1,fM2,max_iter,ftM1,ftM2,fpop1,fpop2,A1,A2,fZ1,fZ2)
    N = size(fM1,1);
...
function M = matrix_convert(fM)
    M = zeros(size(fM,1)*size(fM,2)*size(fM,3),size(fM,4));
...
function [T1,T2,W1,W2] = est_theta(fM1,fM2)
    ...
    %transform all M values into a 2D matrix form
        ...
        for i=1:size(M1,1)
            for j=1:size(M1,2)
                for t=1:size(M1,3)
                    Tt = p1*alpha1(id,:)+squeeze(M1(i,j,t,:))';
        ...
        for i=1:size(M2,1)
            for j=1:size(M2,2)
                for t=1:size(M2,3)
                    Tt = p2*alpha2(id,:)+squeeze(M2(i,j,t,:))';
                    Tt = exp(psi(Tt)-psi(sum(Tt)));
                    Tt(isnan(Tt)) = 0;
                    T2(i,j,t,:) = Tt;
                    id=id+1;
